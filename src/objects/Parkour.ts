...
export default class Parkour {
  ...
  public material: THREE.MeshLambertMaterial = new THREE.MeshLambertMaterial({ color: 0xffffff });

  public normalBlock: THREE.Mesh = new THREE.Mesh(new THREE.BoxGeometry(4, 1, 4), this.material);

  public longBlock: THREE.Mesh = new THREE.Mesh(new THREE.BoxGeometry(4, 1, 8), this.material);
  ...
  public renderParkour(): void {
    this.level1.forEach((obstacle) => {
      SceneManager.scene.add(obstacle.mesh);
