...
export default class Player {
    constructor(index, ai, brain = []) {
        ...
        this.spawnPoint = new THREE.Vector3(0, 0.5, 20);
        this.onGround = false;
        ...
        this.obstacleCoordinations = { current: new THREE.Vector3(), next: new THREE.Vector3() };
        this.finished = false;
        this.inputValues = [];
        this.alive = true;
        this.deathTimer = 5;
        this.highestObstacleIndex = 0;
        this.userFitness = 0;
        this.maxFitness = 0;
        this.loaded = false;
        this.index = index;
    ...
    update(deltaTime, isRace) {
        ...
        const playerVelocity = Math.abs(this.playerBody.velocity.x) +
        ...
        if (this.ai) {
            ...
            if (this.ai) {
                ...
                if (Game.colorMode == 0) {
                    ...
                    const meshMaterial = this.mesh.material;
                    this.mesh.material.color.setRGB(Math.abs(forwardBackward), Math.abs(leftRight), jumpEffect);
                }
    ...
    calculateObstacleDistance() {
        ...
        const maxDistance = Math.sqrt((currentPosition.x - nextPosition.x) ** 2 +
            (currentPosition.y - nextPosition.y) ** 2 +
            (currentPosition.z - nextPosition.z) ** 2);
        const currentDistance = Math.sqrt((this.playerBody.position.x - nextPosition.x) ** 2 +
     ...
     */
    calculateFitness(printScore = false) {
        if (this.ai) {
            this.brain.score = 0;
            // progress in level
