...
class Statistics {
    ...
    renderPerformance(addedX = 0, addedY = 0, inSettings = false) {
        const startPosition = { x: window.innerWidth * 0.015 - (Statistics.visualisationPosition * Number(!inSettings)) + addedX, y: window.innerHeight * 0.04 + addedY };
        const dimensions = { width: window.innerWidth * 0.23, height: window.innerHeight * 0.4 };
        const max = Math.ceil(Math.max(...Statistics.highscores) / 225) * 225;
        let min = 0;
        if (!Game.neat.usePretrainedNetwork) {
            min = Math.min(...Statistics.averageScores);
        }
        const maxHighscore = Math.ceil(Math.max(...Statistics.highscores));
        const maxAverage = Math.ceil(Math.max(...Statistics.averageScores));
        ...
        Statistics.highscores.forEach((score, index) => {
            if (index > 0) {
        ...
        Statistics.averageScores.forEach((score, index) => {
            if (index > 0) {
    ...
    renderOutput(best_player, addedX = 0, addedY = 0, inSettings = false) {
        const startPosition = { x: window.innerWidth * 0.015 - (Statistics.visualisationPosition * Number(!inSettings)) + addedX, y: window.innerHeight * 0.58 + addedY };
        const dimensions = { width: window.innerWidth * 0.23, height: window.innerHeight * 0.4 };
    ...
    renderProgression(addedX = 0, addedY = 0, inSettings = false) {
        const startPosition = { x: window.innerWidth * 0.015 - (Statistics.visualisationPosition * Number(!inSettings)) + addedX, y: window.innerHeight * 0.04 + addedY };
        const dimensions = { width: window.innerWidth * 0.23, height: window.innerHeight * 0.4 };
...
Statistics.highscore = -Infinity;
Statistics.averageScores = [];
Statistics.checkpointsReached = [];
Statistics.previousCheckpointsReached = [];
Statistics.startHidingGraphs = true;
Statistics.visualisationHidden = false;
Statistics.visualisationPosition = 0;
Statistics.hidingDuration = 1000;
