...
class Game extends Scene {
    constructor() {
        ...
        this.pmremGenerator = new THREE.PMREMGenerator(MainCanvas.renderer);
            ...
            if (!this.openEditor) {
                ...
                if (MouseListener.buttonPressed(0)) {
                    ...
                    if (UICollision.checkSquareCollisionMult(((window.innerWidth * 0.485) - Statistics.visualisationPosition) / window.innerWidth, 0.929, 0.1, 0.05) && !this.race.finished) {
                        if (this.race.isRaceReady || (Game.neat.players.filter(player => player.finished).length) / (Math.floor(Game.neat.neat.popsize)) >= 0.84) {
    ...
    resetRace(restart) {
        ...
        if (restart) {
            ...
            Parkour.levels[Parkour.activeLevel].resetCamera();
        }
    }
    update(deltaTime) {
    ...
    updateLight() {
        const bestPlayer = Game.alivePlayers.reduce((prev, current) => (prev.brain.score > current.brain.score) ? prev : current);
        MainCanvas.directionalLight.position.set(bestPlayer.mesh.position.x + 70, bestPlayer.mesh.position.y + 140, bestPlayer.mesh.position.z - 140);
        MainCanvas.directionalLight.target.position.set(bestPlayer.mesh.position.x, bestPlayer.mesh.position.y, bestPlayer.mesh.position.z);
